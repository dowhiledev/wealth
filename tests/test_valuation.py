...
def test_fifo_valuation(tmp_db_path):
    ...
    with session_scope(cfg.db_path) as s:
        ...
        # Price after
        upsert_price(s, asset_symbol="BTC", quote_ccy="USD", ts=t0 + timedelta(days=3), price=Decimal("30000"))

    with session_scope(cfg.db_path) as s:
        positions, totals = summarize_portfolio(s, as_of=t0 + timedelta(days=4), quote="USD")

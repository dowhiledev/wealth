...
from wealth.db.repo import session_scope
...
def generate_pdf_report(
    ...
    *,
    ...
    data.append(["Totals", "-", "-", str(totals["value"]), str(totals["cost_open"]), str(totals["unrealized"]), str(totals["realized"])])
    ...
    story += [Paragraph("Portfolio Summary", styles["Heading2"]), table, Spacer(1, 0.5 * cm)]

    # Charts
    if allocation_img and allocation_img.exists():
        story += [Paragraph("Allocation", styles["Heading2"]), Image(str(allocation_img), width=16 * cm, height=12 * cm), Spacer(1, 0.5 * cm)]
    if value_img and value_img.exists():
        story += [Paragraph("Portfolio Value", styles["Heading2"]), Image(str(value_img), width=16 * cm, height=9 * cm), Spacer(1, 0.5 * cm)]
    if pnl_img and pnl_img.exists():
        story += [Paragraph("Realized PnL", styles["Heading2"]), Image(str(pnl_img), width=16 * cm, height=9 * cm), Spacer(1, 0.5 * cm)]
